...
export async function issueCreditNote({ clientEmail, invoice_id, item, reservationCode }: { clientEmail: string, invoice_id: string, item: AccommodationItem, reservationCode: string }) {
    ...
    console.log(invoice,creditNote);
    ...
    if (property.invoicing_nif && property.default_series) {
        ...
        if (newInvoice.id) {
            ...
            for (const fee of fees) {
                ...
                if (refund_amount == 50) {
                    if (fee.fee.type.toLowerCase() == 'accommodation') {
                        const price = (fee.amount_net_total / 2) || 0;
                        const vat = fee.amount_tax_total ? Number((fee.amount_tax_total / fee.amount_net_total).toFixed(2)) * 100 : 0;
                        const qty = 1;
                        const query: Record<string, string | number> = {
                            id,
                            product_id,
