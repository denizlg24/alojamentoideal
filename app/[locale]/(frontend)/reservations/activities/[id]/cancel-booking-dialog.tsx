...
import { cancelActivityBooking } from "@/app/actions/getExperience";
...
export const CancelBookingDialog = ({
  ...
  const cancel = async () => {
    setError("");
    setLoading(true);
    const response = await cancelActivityBooking({
        ...
        <DialogHeader>
          ...
          <div className="grid min-[420px]:grid-cols-2 grid-cols-1 w-full gap-x-4 gap-y-2">
            ...
            disabled={loading}
              ...
              variant={"default"}
            >
                {loading ? <><Loader2 className="animate-spin"/> {t("cancelling")}</> :t("cancel-reservation") }
            </Button>
            <DialogClose asChild>
